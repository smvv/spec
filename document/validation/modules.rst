.. _valid-module:
.. index:: modules, type definition, function type, function, table, memory, global, element, data, start function, import, export, vector, context
...
* The length of the vector :math:`C.\TABLES` must not be larger than :math:`1`.

* The length of the vector :math:`C.\MEMS` must not be larger than :math:`1`.
...
.. note::
   The definition of the :ref:`context <context>` :math:`C` in this rule is recursive:
   ...
   However, all types needed to construct :math:`C` can easily be determined from a simple pre-pass over the module without performing any actual validation.

.. note::
   The restriction on the number of tables and memories may be lifted in future versions of WebAssembly.

.. todo:: Limit available context for globals
...
Expressions :math:`\expr` are classified by :ref:`result types <syntax-resulttype>` :math:`\resulttype`.
...
Functions :math:`\func` are classified by :ref:`function types <syntax-functype>` :math:`\functype`.
...
* Let :math:`C'` be the same :ref:`context <context>` as :math:`C`,
  but with the |LOCALS| vector set to the :ref:`value types <syntax-valtype>` :math:`[t_1^\ast~t^\ast]`
  and the |LABELS| vector set to the singular vector of :ref:`result types <syntax-valtype>` :math:`[(t_2^\ast)]`.
...
Tables :math:`\table` are classified by :ref:`table types <syntax-tabletype>` :math:`\tabletype`.
...
Memories :math:`\mem` are classified by :ref:`memory types <syntax-memtype>` :math:`\memtype`.
...
Globals :math:`\global` are classified by :ref:`global types <syntax-globaltype>` :math:`\globaltype`.
...
Exports :math:`\export` and export descriptions :math:`\exportdesc` are classified by their export :ref:`name <syntax-name>`.
...
* The mutability annotation :math:`\MUT^?` must be empty (that is, :math:`x` must be immutable).
...
Imports :math:`\import` and import descriptions :math:`\importdesc` are classified by :ref:`external types <syntax-externtype>` :math:`\externtype`.
...
* The mutability annotation :math:`\MUT^?` must be empty.
