...
let invoke (clos : closure) (vs : value list) : value list =
  eval_func clos vs no_region

let eval_const (inst : instance) (const : const) at =
  let c = config [] (List.map plain const.it) in
  match eval inst c with
  | [v] -> v
  | vs -> Crash.error at "wrong number of results on stack"
...
let init_table (inst : instance) (seg : table_segment) =
  ...
  let offset = i32 (eval_const inst e e.at) e.at in
