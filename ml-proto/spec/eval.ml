...
(*
 * Evaluation contexts C follow this grammar:
 *
 * (eval context)  C ::= [] | v* C e* | (label{target} C) | (local{locals} C)
 *
 * which can be refactored into
 *
 * (block context) E ::= v* [] e*
 * (call context)  B ::= b*[v* [] e*]
 * (eval context)  C ::= c*[b*[v* [] e*]]
 * (block)         b ::= E[label{target} []]
 * (call)          c ::= B[local{locals} []]
 *
 * which the following type definitions represent, and which interprets contexts
 * as a stack of blocks nested into a stack of calls, which is a better fit
...
let eval_instr (c : config) (e : instr) (es, vs, bs, cs : eval_context) : eval_context =
  ...
  | Br (n, x), vs, b :: bs', _ when x.it = 0 ->
    let es', vs' = b.bcontext in
    b.target @ es', take n vs e.at @ vs', bs', cs

  | Br (n, x), vs, b :: bs', _ ->
    [Br (n, (x.it - 1) @@ x.at) @@ e.at], vs, bs', cs
